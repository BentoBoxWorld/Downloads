...
- Endpoints: `GET /api/blog/posts?page&tag&q` (paginated public list with optional tag filter and case-insensitive substring search across title/summary/body), `GET /api/blog/posts/:slug` (public, bumps view count), `GET /api/blog/tags` (aggregated tag counts across published posts), `GET /api/blog/comments-config` (public Giscus config or null), `GET /api/blog/feed.xml` (RSS 2.0), `GET /blog/sitemap.xml` (sitemap), `GET /blog/highlight.css` (highlight.js theme served from the bundled github-dark theme with our background override). Author endpoints under `/api/blog/admin/posts` (CRUD), `/api/blog/admin/posts/:id/publish` (with optional `{ at: <epoch_ms> }` body to schedule), `/api/blog/admin/posts/:id/unpublish`, `/api/blog/admin/images` for multipart uploads (5 MB cap, PNG/JPG/WebP/GIF only). Admin-only: `GET /api/admin/authors` and `POST /api/admin/authors` for granting/revoking the `canAuthorBlog` flag (admins are implicit authors and can't be revoked from there). All writes require `X-Csrf-Token` and `requireBlogAuthor` (or `requireAdmin` for the authors endpoints).
...
- Comments: optional Giscus integration. When `env.giscus` is present (with `repo`, `repoId`, `category`, `categoryId`), each post page embeds the widget below the body. The CSP allowlist for `giscus.app` (script/frame/connect) is added unconditionally; nothing renders when `env.giscus` is missing because `/api/blog/comments-config` returns `null`.
- Phase scope: Phase 1 = list + permalink + RSS + OG + Discord webhook + image uploads + draft/publish, single author. Phase 2 added tags + tag archive, code highlighting, GFM callouts, scheduled publishing, sitemap. Phase 3 (current) adds the multi-author grant UI in admin (Authors tab), public substring search across title/summary/body, and opt-in Giscus comments. Phase 4 candidates: dev.to cross-post button, nightly git backup of post markdown, Plausible/Umami analytics, Mermaid diagrams, series support.
